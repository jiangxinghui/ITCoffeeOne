...
//--------------------------- CurrentPhase -----------------------------//
//----------------------------------------------------------------------//
CurrentPhase::CurrentPhase(int index,  Phase* phase, uint32_t timeInPhase, const ShotSnapshot& shotSnapshotAtStart) : index(index), phase{ phase }, timeInPhase(timeInPhase), shotSnapshotAtStart{ &shotSnapshotAtStart } {}
...
void PhaseProfiler::updatePhase(uint32_t timeInShot, SensorState& state) {
  ...
  if (phaseIdx >= profile.phaseCount() || profile.globalStopConditions.isReached(state, timeInShot)) {
    ...
    Phase* phase=profile.phases.get(phaseIdx);

    currentPhase.update(currentPhaseIdx - 1, phase, timeInPhase);
    return;
  }

  if (!profile.phases.get(phaseIdx)->isStopConditionReached(state, timeInShot, phaseChangedSnapshot)) {
    currentPhase.update(phaseIdx, profile.phases.get(phaseIdx), timeInPhase);
    return;
  }
...
void PhaseProfiler::reset() {
  ...
  currentPhase.update(0, profile.phases.get(0), 0);
}
